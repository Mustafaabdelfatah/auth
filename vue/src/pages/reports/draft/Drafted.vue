    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:item.sites="{ item }">
          <v-chip label small color="primary" v-for="(site, index) in item.sites" :key="index"
        ...
        <template v-slot:item.action="{ item }">
          <div class="actions">
            <v-btn dark small icon color="primary" :to="`/reports/drafted/show/${item.id}`" v-can="'update-user'">
              <v-icon>mdi-eye</v-icon>
            </v-btn>
            <v-btn icon small dark color="green" :to="`/reports/drafted/edit/${item.id}`">
