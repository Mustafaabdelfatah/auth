    ...
    <v-card>
      ...
      <v-data-table show-select v-model="selected" :headers="headers" :items="items" :options.sync="options"
        ...
        <template v-slot:item.data-table-select="{ item, isSelected, select }">
          <v-simple-checkbox v-if="item.active != 1 && item.id != 1" :value="isSelected" :readonly="item.disabled"
            :disabled="item.disabled" @input="select($event)"></v-simple-checkbox>
        </template>
        ...
        <template v-slot:item.globalDate="{ item }">
          <v-chip label small v-if="item.global_date" class="font-weight-bold ma-1">
            {{ $t("reports.global") }}
          </v-chip>
        ...
        <template v-slot:item.dashboard="{ item }">
          <div class="d-flex justify-center">
            <v-switch v-model="item.active" color="primary" :value="item.active" :disabled="item.active === 1"
              hide-details @click="updateActive(item)"></v-switch>
          </div>
        </template>

        <template v-slot:item.action="{ item }">
          <div class="actions">
            <v-btn dark small icon color="primary" :to="`/reports/pinned/show/${item.id}`" v-can="'update-user'">
              <v-icon>mdi-eye</v-icon>
            </v-btn>
            <v-btn dark small icon color="green" :to="`/reports/pinned/edit/${item.id}`">
