<template>
    <div class="ps-lg-5 pe-lg-6">
        <Tabs :tabs="tabs" initialTab="first" :slotLabel="true" :grow="true" v-model="currentTap">
            <template #stepOne>
                <div class="single-step">
                    ...
                    <div class="tab-title">
                        <p>{{ t('buttons.step') }} {{ locale === 'ar' ? useNumConverter(1) : 1 }}</p>
                        <span>
            ...
            <template #stepTwo>
                <div class="single-step">
                    ...
                    <div class="tab-title">
                        <p>{{ t('buttons.step') }} {{ locale === 'ar' ? useNumConverter(2) : 2 }}</p>
                        <span>
            ...
            <template #stepThree>
                <div class="single-step">
                    ...
                    <div class="tab-title">
                        <p>{{ t('buttons.step') }} {{ locale === 'ar' ? useNumConverter(3) : 3 }}</p>
                        <span>
            ...
            <template v-slot:first>
                ...
                <BasicDataStep v-model="employeeBasicData" />
            </template>
            <template v-slot:second>
                <h3 class="step-title">
                    {{ t('buttons.internal_data') }}
                </h3>
                <InternalDataStep v-model="employeeInternalData" />
            </template>
        ...
        <div class="generic-form-actions d-flex align-center justify-end ga-3 w-100">
            ...
            <v-btn @click="save" v-ripple="{ class: `text-secondary` }" :disabled="currentTap !== 'third'"
                class="d-flex px-5 text-white border-secondary bg-secondary" flat height="42" type="button"
                :loading="appStore.buttonLoading">
                {{ t('buttons.add') }}
            </v-btn>
        </div>
    </div>
...
import { createUsers } from '@/services/apis/users.js'
...
const save = async () => {
    ...
    await createUsers(allData).finally(() => {
...
watch(
    ...
    (newValue) => {
        const form = transformSchemaToObject(newValue)
        const isValid = computed(() => validateObject(form, ignoreFields.value))
...
::v-deep {
    ...
    .v-tabs {
        ...
        .v-btn.v-tab-item--selected {
            border-color: rgba(var(--v-theme-tabsActiveBorder), 0.4);
            background-color: rgba(var(--v-theme-primary), 1);
