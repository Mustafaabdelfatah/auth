<template>
  <v-card class="min-w-0">
    <v-text-field v-model="filter" class="pa-1 py-2 todo-filter elevation-1" placeholder="Filter tasks"
      prepend-inner-icon="mdi-magnify" hide-details block clearable solo flat></v-text-field>
    ...
    <v-slide-y-transition v-else group tag="div">
      ...
      >
        ...
        >
          ...
          <div>
            <v-chip v-for="label in task.labels" :key="label" :color="getLabelColor(label)"
              class="font-weight-bold mt-1 mr-1" outlined x-small>
              {{ getLabelTitle(label) }}
            </v-chip>
          </div>
        </div>
