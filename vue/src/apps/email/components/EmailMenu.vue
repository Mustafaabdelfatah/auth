<template>
  <div>
    ...
    <v-list dense nav class="mt-2 pa-0">
      <v-list-item v-for="(item, index) in menu" :key="index" :to="item.link" active-class="primary--text" link>
        <v-list-item-icon>
        ...
        <v-list-item-action v-if="item.count > 0">
          <v-badge inline color="primary" class="font-weight-bold" :content="item.count">
          </v-badge>
        </v-list-item-action>
      </v-list-item>
    </v-list>
    ...
    <v-list dense nav class="mt-2 pa-0">
      ...
      <v-list-item v-for="(item, index) in labels" :key="index" :to="item.link" exact active-class="primary--text" link>
        <v-list-item-icon>
        ...
        <v-list-item-action v-if="item.count > 0">
          <v-badge inline :content="item.count" :color="item.color" class="font-weight-bold">
          </v-badge>
        </v-list-item-action>
      </v-list-item>
    </v-list>
    ...
    <email-compose
      ...
      @close-dialog="showCompose = false"
    />
  </div>
</template>
